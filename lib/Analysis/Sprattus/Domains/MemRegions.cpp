...
#include "Analysis/Sprattus/Core/ParamStrategy.h"
...
DomainConstructor::Register
    NoAlias("NoAlias",
            ...
            [](const FunctionContext& fctx, llvm::BasicBlock* loc, bool after) {
                return params::ForPointerPairs<sprattus::domains::NoAlias>(
                    fctx, loc, after, true);
            });
...
DomainConstructor::Register ValidRegion(
    "ValidRegion", " domain for expressing whether a pointer points to a valid"
                   " memory region",
    [](const FunctionContext& fctx, llvm::BasicBlock* loc, bool after) {
        return params::ForPointers<sprattus::domains::ValidRegion>(fctx, loc,
                                                                   after);
    });
...
DomainConstructor::Register ConstRegion(
    ...
    [](const FunctionContext& fctx, llvm::BasicBlock* loc, bool after) {
        return params::ForPointers<sprattus::domains::ConstantRegion>(fctx, loc,
                                                                      after);
    });
...
DomainConstructor::Register VarRegion(
    ...
    [](const FunctionContext& fctx, llvm::BasicBlock* loc, bool after) {
        return ForPointerSizePairs<sprattus::domains::VariableRegion>(fctx, loc,
                                                                      after);
    });
