...
#include "Analysis/SymbolicAbstraction/Core/Fragment.h"
#include "Analysis/SymbolicAbstraction/Core/FragmentDecomposition.h"
...
#include "Analysis/SymbolicAbstraction/Core/FloatingPointModel.h"
#include "Analysis/SymbolicAbstraction/Utils/Config.h"
#include "Analysis/SymbolicAbstraction/Core/ModuleContext.h"
      ...
      Config_(ModuleContext_->getConfig()), DominatorTreePass_(nullptr)
{
    ...
    for (auto& entry : *Function_->getValueSymbolTable()) {
        ...
        if (type->isFloatingPointTy() &&
            FloatingPointModel_->supportsType(type)) {
            represented.insert(val);
        }
    }
    ...
    if (Config_.get<bool>("FunctionContext", "RepresentGlobals", true)) {
        ...
        for (auto* local : represented) {
            while (global_names.find(local->getName().str()) !=
                   global_names.end()) {
...
// outgoing edges.
z3::expr FunctionContext::getBasicBlockBodyCondition(const Fragment& frag,
...
z3::expr FunctionContext::getBasicBlockPhiCondition(const Fragment& frag,
                                                    llvm::BasicBlock* bb) const
{
    z3::expr result = getZ3().bool_val(false);
...
z3::expr FunctionContext::formulaFor(const Fragment& frag) const
{
    ...
    for (llvm::BasicBlock* loc : frag.locations()) {
        for (auto edge : frag.edgesFrom(loc)) {
            auto vm_pre =
                ValueMapping::before(*this, frag, loc->getTerminator());
    ...
    for (llvm::Value* value : representedValues()) {
        if (!frag.defines(value)) {
            preservation_formula = preservation_formula &&
                                   vm_start.getFullRepresentation(value) ==
    ...
    for (BasicBlock* bb : frag.locations()) {
        ...
        if (bb != frag.getStart()) {
            cfg_formula =
                cfg_formula && implies(getBasicBlockBodyCondition(frag, bb),
    ...
    for (BasicBlock* bb_prev : frag.locations()) {
        ...
        for (auto itr = succ_begin(bb_prev); itr != succ_end(bb_prev); ++itr) {
            ...
            for (auto itr = succ_begin(bb_prev); itr != succ_end(bb_prev);
                 ++itr) {
            ...
            cfg_formula =
                cfg_formula && implies(getEdgeVariable(bb_prev, bb1), e);
        }
    }
    ...
    return code_formula && preservation_formula && mem_transfer &&
           cfg_formula && undef_formula && init_mem;
}
