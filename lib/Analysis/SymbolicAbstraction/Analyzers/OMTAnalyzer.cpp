...
#include "Analysis/SymbolicAbstraction/Core/ConcreteState.h"
#include "Analysis/SymbolicAbstraction/Core/ValueMapping.h"
#include "Analysis/SymbolicAbstraction/Domains/Intervals.h"
...
OMTAnalyzer::OptimizeStatus OMTAnalyzer::runOptimize(
    const z3::expr& objective, const z3::expr& phi, const ValueMapping& vmap,
    AbstractValue* target, bool maximize, unsigned timeout_ms) const
{
                                       ...
                                       const ValueMapping& vmap) const
{
    ...
    for (auto& obj : objectives) {
        ...
        if (max_res == OptimizeStatus::Unsat ||
            min_res == OptimizeStatus::Unsat) {
            bool was_bottom = result->isBottom();
            result->resetToBottom();
            return !was_bottom;
        }
