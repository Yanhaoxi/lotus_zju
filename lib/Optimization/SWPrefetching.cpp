                   ...
                   for(size_t index=0 ; index<NextPhiDependencies.size() ; index++){
                        if(NextPhiDependencies[index]->getOpcode()==Instruction::GetElementPtr){
                           if((NextPhiDependencies[index]->getOperand(0)==curDep || NextPhiDependencies[index]->getOperand(1)==curDep )){
                               NewInstr =dyn_cast<GetElementPtrInst>(Builder.CreateInBoundsGEP(curDep->getType(), curDep,curDep->getType()->isIntegerTy(64) ? ConstantInt::get(Type::getInt64Ty((curDep->getParent())->getContext()),prefetchDist): ConstantInt::get(Type::getInt32Ty((curDep->getParent())->getContext()),prefetchDist))); 
                    ...
                    NewInstr =dyn_cast<GetElementPtrInst>(Builder.CreateInBoundsGEP(phi->getType(), phi,ConstantInt::get(Type::getInt64Ty((curDep->getParent())->getContext()),prefetchDist))); 
...
bool SWPrefetchingLLVMPass::InjectPrefechesOnePhiPartTwo(Instruction* I, LoopInfo &LI,Instruction*  Phi, SmallVector<Instruction*,20> &DepInstrs, int64_t prefetchDist){
 ...
 if(nonCanonical){
   ...
   newPhi =dyn_cast<GetElementPtrInst>(Builder.CreateInBoundsGEP(phi->getType(), phi,phi->getType()->isIntegerTy(64) ? ConstantInt::get(Type::getInt64Ty(((phi->getFunction())->getParent())->getContext()),prefetchDist): ConstantInt::get(Type::getInt32Ty(((phi->getFunction())->getParent())->getContext()),prefetchDist)));
