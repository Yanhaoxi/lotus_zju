...
//===----------------------------------------------------------------------===//
/// @file SWPrefetching.cpp
/// @brief Software Prefetching pass implementation
///
/// This file implements a profile-guided software prefetching pass based on
/// the PT-GET paper (EuroSys'22). The pass injects prefetch instructions
/// into loops to reduce memory access latency.
///
/// The pass works by:
/// 1. Loading profile data (FDO, LBR, or LLM-based)
/// 2. Identifying loop-carried induction variables that feed loads
/// 3. Computing prefetch distances based on profile hints
/// 4. Cloning dependence chains to compute future addresses
/// 5. Inserting llvm.prefetch instructions at the right place
///
/// @par Related Work:
/// - CGO 17: Software Prefetching for Indirect Memory Accesses
/// - TOCS 19: Software Prefetching for Indirect Memory Accesses
...
static cl::opt<std::string>
    PrefetchFile("input-file", cl::desc("Specify input filename for mypass"),
...
/// @brief Fetch the profile hint (call target map) associated with the debug
/// location of `Inst`.
/// @param Inst The instruction to get debug location from
/// @param TopSamples The top-level function samples to search
/// @return The call target map if found, or an empty error_code
ErrorOr<Hints> getHints(const llvm::Instruction &Inst,
                        const llvm::sampleprof::FunctionSamples *TopSamples) {
...
bool SWPrefetchingLLVMPass::doInitialization(Module &M) {
  if (PrefetchDistanceProviderMode == PrefetchDistanceProvider::Profile) {
    ...
    ErrorOr<std::unique_ptr<SampleProfileReader>> ReaderOrErr =
        SampleProfileReader::create(PrefetchFile, Ctx);
    if (std::error_code EC = ReaderOrErr.getError()) {
      std::string Msg = "Could not open profile: " + EC.message();
      Ctx.diagnose(DiagnosticInfoSampleProfile(PrefetchFile, Msg,
    ...
    for (auto &F : M) {
      const llvm::sampleprof::FunctionSamples *SamplesReaded =
          Reader->getSamplesFor(F);
      if (SamplesReaded) {
        AutoFDOMapping = true;
      }
    }
...
// update.
CmpInst *
SWPrefetchingLLVMPass::getCompareInstrGetElememntPtr(Loop *L,
                                                     Instruction *nextInd) {
  SetVector<Instruction *> BBInsts;
  auto *B = L->getExitingBlock();
  int count = 0;
...
// induction variable.
ConstantInt *
SWPrefetchingLLVMPass::getValueAddedToIndVar(Loop *L, Instruction *nextInd) {
  bool Changed = false;
...
bool SWPrefetchingLLVMPass::InjectPrefeches(
    ...
    SmallVector<llvm::Instruction *, 10> &CapturedPhis,
    SmallVector<llvm::Instruction *, 10> &CapturedLoads,
          ...
          if (CheckLoopCond(PhiLoop)) {
            if (GetIncomingValue(PhiLoop, phi)) {
              ...
              if (IncInstr->getOpcode() == Instruction::Mul &&
                ...
                if (dyn_cast<ConstantInt>(IncInstr->getOperand(1))) {
                  // errs()<< "   Operand#1: "<<
                  // *(dyn_cast<ConstantInt>(IncInstr->getOperand(1))) << "\n";
                } // getOperand(1)
              } // MUL
          ...
          if (PEB) {
            ...
            if (DepPhiInsts.size() > 0) {
              llvm::Instruction *insertPt =
                  phi->getParent()->getFirstNonPHIOrDbg();
...
bool SWPrefetchingLLVMPass::InjectPrefechesOnePhiPartTwo(
  ...
  if (nonCanonical) {
    ...
    newPhi = dyn_cast<GetElementPtrInst>(Builder.CreateInBoundsGEP(
        phi->getType(), phi,
...
bool SWPrefetchingLLVMPass::InjectPrefechesOnePhiPartOne(
    Instruction *I, LoopInfo &LI, SmallVector<llvm::Instruction *, 10> &Phi,
    SmallVector<llvm::Instruction *, 10> &CapturedLoads,
    SmallVector<Instruction *, 20> &DepInstrs, int64_t prefetchDist,
    bool ItIsIndirectLoad) {
  bool done = false;
        ...
        for (long unsigned int j = 0; j < AllCurLoads.size(); j++) {
          if (AllCapturedInstrs[i].size() == AllCapturedInstrs[j].size() &&
            ...
            if (AllCapturedLoads[i].size() == AllCapturedLoads[j].size() &&
              ...
              if (!(std::find(correctMapping.begin(), correctMapping.end(),
                              i) != correctMapping.end())) {
            ...
            if (IsDep(curLoad, LI, phi, DependentLoadsToCurLoadx,
              ...
              if (DependentLoadsToCurLoadx.size() > 0) {
                ...
                for (auto &s : DependentLoadsToCurLoadx) {
                  for (long unsigned int i = 0; i < AllLoadsDepToPhix.size();
                       i++) {
