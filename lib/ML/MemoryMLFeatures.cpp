...
MLFeaturesCallSite::MLFeaturesCallSite()
    : m_cs(nullptr), callee_nodes(0), callee_collapsed(0), callee_accessed(0),
      callee_sequence(0), callee_alloca(0), callee_heap(0), callee_external(0),
      callee_pointers(0), callee_alloc_sites(0), callee_mem_accesses(0),
      callee_safe_alloc_sites(0),
      ////
      td_callee_nodes(0), td_callee_collapsed(0), td_callee_accessed(0),
      td_callee_sequence(0), td_callee_alloca(0), td_callee_heap(0),
      td_callee_external(0), td_callee_pointers(0), td_callee_alloc_sites(0),
      td_callee_mem_accesses(0), td_callee_safe_alloc_sites(0) {}

MLFeaturesCallSite::MLFeaturesCallSite(CallBase &CB)
    : m_cs(&CB), callee_nodes(0), callee_collapsed(0), callee_accessed(0),
      callee_sequence(0), callee_alloca(0), callee_heap(0), callee_external(0),
      callee_pointers(0), callee_alloc_sites(0), callee_mem_accesses(0),
      callee_safe_alloc_sites(0),
      ////
...
void MLFeaturesCallSite::write(raw_ostream &o) const {
  if (m_cs) {
    ...
    if (callee_nodes > 0) {
      o << "  Number of pointers per node: " << callee_pointers / callee_nodes
        << "\n";
    }
...
public:
  ...
  std::unique_ptr<Graph> getTopDownGraph(DsaCallSite &CS) {
    ...
    if (callee && caller && m_bu_graphs->hasGraph(*callee) &&
        m_bu_graphs->hasGraph(*caller)) {
   ...
   **/
  Graph &getSummaryGraph(const Function &F) { return m_bu_graphs->getGraph(F); }

  MLFeaturesCallSite extractMLFeatures(CallBase &CB) {
    auto it = m_cs_features_map.find(&CB);
    ...
    MLFeaturesCallSite res(CB);
    /// -- before td-unification
    for (const Node &n :
         llvm::make_range(callee_graph.begin(), callee_graph.end())) {
      ++res.callee_nodes;
    ...
    res.callee_alloc_sites = std::distance(callee_graph.alloc_sites().begin(),
                                           callee_graph.alloc_sites().end());
    ...
    for (const Node &n :
         llvm::make_range(td_callee_graph->begin(), td_callee_graph->end())) {
      ++res.td_callee_nodes;
      if (n.isOffsetCollapsed()) {
    ...
    res.td_callee_pointers = std::distance(td_callee_graph->scalar_begin(),
                                           td_callee_graph->scalar_end()) +
                             std::distance(td_callee_graph->formal_begin(),
                                           td_callee_graph->formal_end()) +
                             std::distance(td_callee_graph->return_begin(),
                                           td_callee_graph->return_end());
    res.td_callee_alloc_sites =
        std::distance(td_callee_graph->alloc_sites().begin(),
                      td_callee_graph->alloc_sites().end());

    if (IncludeExpensiveFeatures) {
      getMemoryAccessesFeatures(
          *td_callee_graph, *(const_cast<Function *>(callee)),
          res.td_callee_mem_accesses, res.td_callee_safe_alloc_sites);
    }

    m_cs_features_map.insert({&CB, res});
    return res;
  }
};
    ...
    for (auto &B : F) {
      for (auto &I : B) {
        if (auto *CB = dyn_cast<CallBase>(&I)) {
          auto features = extractMLFeatures(*CB);
          features.write(errs());
        }
      }
    }
...
MLFeaturesCallSite MemoryMLFeaturesPass::extractMLFeatures(CallBase &CB) {
  return m_impl->extractMLFeatures(CB);
}
