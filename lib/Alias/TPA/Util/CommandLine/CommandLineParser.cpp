#include "Alias/TPA/Util/CommandLine/CommandLineParser.h"
...
void CommandLineParser::addOptionalFlagEntry(const StringRef &name,
                                             OptionalFlagEntry &&entry) {
  auto inserted = optFlagMap.try_emplace(name, std::move(entry)).second;
  assert(inserted && "Duplicate entry in optFlagMap!");
}

void CommandLineParser::addPositionalFlagEntry(const StringRef &name,
                                               PositionalFlagEntry &&entry) {
  assert(std::find_if(posFlagMap.begin(), posFlagMap.end(),
                      [&name](auto const &pair) {
                        return name == pair.first;
                      }) == posFlagMap.end() &&
         "Duplicate entry in posFlagMap!");

  posFlagMap.emplace_back(std::make_pair(name, std::move(entry)));
}

void CommandLineParser::addPositionalFlag(const StringRef &name,
...
void CommandLineParser::parseArgs(const std::vector<StringRef> &args,
                                  CommandLineFlags &result) const {
  ...
  while (numFlagConsumed < args.size()) {
    ...
    if (!isOption(flag)) // Positional arguments
    {
      ...
      auto inserted =
          result.addFlag(posFlagMap[numPositionalConsumed].first, flag);
      ...
      if (itr->second.defaultValue) {
        ...
        if (numFlagConsumed >= args.size())
          throw ParseException(
              "More argument needs to be provided after option " + flag.str());
        auto flagArg = args[numFlagConsumed];
        if (isOption(flagArg))
          throw ParseException(
              "Found another option instead of an argument after option " +
              flag.str());
...
CommandLineFlags CommandLineParser::parseCommandLineOptions(int argc,
                                                            char **argv) const {
  ...
  try {
    ...
    errs() << "Please run \"" << argv[0]
           << " -help\" to view usage information\n";
    std::exit(-1);
