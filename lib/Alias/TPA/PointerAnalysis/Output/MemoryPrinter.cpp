...
#include "Alias/TPA/Util/Iterator/DereferenceIterator.h"
#include "Alias/TPA/Util/Iterator/InfixOutputIterator.h"
...
raw_ostream &operator<<(raw_ostream &os, const PtsSet &pSet) {
  os << "{ ";
  std::copy(util::deref_const_iterator(pSet.begin()),
            util::deref_const_iterator(pSet.end()),
            infix_ostream_iterator<MemoryObject>(os, ", "));
  os << " }";
  return os;
}
...
raw_ostream &operator<<(raw_ostream &os, const tpa::ArrayLayout &layout) {
  ...
  std::copy(layout.begin(), layout.end(),
            infix_ostream_iterator<ArrayTriple>(os, ", "));
  os << "}";
  return os;
}
