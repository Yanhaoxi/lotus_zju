...
std::pair<bool, bool> TransferFunction::evalReturnValue(const context::Context* ctx, const ReturnCFGNode& retNode, const ProgramPoint& retSite)
{
	...
	if (retVal == nullptr)
	{
		...
		{
			const auto *ptr = globalState.getPointerManager().getOrCreatePointer(retSite.getContext(), dstVal);
	...
	const auto *dstPtr = ptrManager.getOrCreatePointer(retSite.getContext(), dstVal);
