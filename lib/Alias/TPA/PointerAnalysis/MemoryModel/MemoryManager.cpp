...
const MemoryObject* MemoryManager::allocateGlobalMemory(const llvm::GlobalVariable* value, const TypeLayout* type)
{
	...
	const auto *memBlock = allocateMemoryBlock(AllocSite::getGlobalAllocSite(value), type);
...
{
	const auto *memBlock = allocateMemoryBlock(AllocSite::getFunctionAllocSite(f), TypeLayout::getPointerTypeLayoutWithSize(0));
...
{
	const auto *memBlock = allocateMemoryBlock(AllocSite::getStackAllocSite(ctx, ptr), type);
	return getMemoryObject(memBlock, 0, startWithSummary(type));
}

const MemoryObject* MemoryManager::allocateHeapMemory(const Context* ctx, const llvm::Value* ptr, const TypeLayout* type)
{
	const auto *memBlock = allocateMemoryBlock(AllocSite::getHeapAllocSite(ctx, ptr), type);
	return getMemoryObject(memBlock, 0, true);
}

const MemoryObject* MemoryManager::allocateArgv(const llvm::Value* ptr)
{
	const auto *memBlock = allocateMemoryBlock(AllocSite::getStackAllocSite(Context::getGlobalContext(), ptr), TypeLayout::getByteArrayTypeLayout());
