...
NodeIndex AndersNodeFactory::createObjectNode(const NodeIndex base,
...
unsigned
AndersNodeFactory::constGEPtoFieldNum(const llvm::ConstantExpr *expr) const {
  assert(expr->getOpcode() == Instruction::GetElementPtr &&
         "constGEPtoVariable receives a non-gep expr!");

  unsigned offset = getGEPOffset(expr, dataLayout);
  const Value *underlying = getUnderlyingObject(expr, 6);
  return offsetToFieldNum(underlying, offset, dataLayout, structAnalyzer,
                          module);
}
