...
static bool expandNestedGEP(Function &F, IRBuilder<NoFolder> &builder) {
    ...
    } while (!fixPoint);  // we need a fixed point algorithm here because context expression can be recursive
        ...
        for (auto &I : BB) {
            if (auto *GEP = dyn_cast<GetElementPtrInst>(&I)) {
                ...
                if (!isa<Constant>(GTI.getOperand())) {
                    ...
                    lastBasePtr = builder.CreateGEP(GEP->getSourceElementType(), lastBasePtr, GTI.getOperand());
                ...
                for (gep_type_iterator GTE = gep_type_end(GEP); GTI != GTE; GTI++) {
                    ...
                    if (isa<Constant>(op)) {
                        ...
                        if (consIndices.size() != 1) {
                            ...
                            lastBasePtr = builder.CreateGEP(lastBasePtr->getType()->getPointerElementType(), lastBasePtr, consIndices);
                            LOG_TRACE("{}", *lastBasePtr);
                        }
                        // emit current variable index
                        lastBasePtr = builder.CreateGEP(lastBasePtr->getType()->getPointerElementType(), lastBasePtr, {zero, op});
                        LOG_TRACE("{}", *lastBasePtr);
