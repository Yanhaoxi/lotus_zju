    ...
    for (Function &F : M) {
        ...
        if (F.hasFnAttribute(Attribute::AllocSize)) {
            ...
            if (calloc == nullptr) {
                ...
                FunctionCallee tmp = M.getOrInsertFunction("calloc", i8ptr,
                                                           elementSize->getType(), elementNum->getType());
                calloc = cast<Function>(tmp.getCallee());
                calloc->addRetAttr(Attribute::NoAlias);
            }
        ...
        if (ctxNameRef.startswith("std::allocator_traits<")) {
            if (baseNameRef.startswith("allocate")) {
                ...
                if (calloc == nullptr) {
                    // void* calloc (size_t num, size_t size);
                    FunctionCallee tmp = M.getOrInsertFunction("calloc", i8ptr, num->getType(), num->getType());
                    calloc = cast<Function>(tmp.getCallee());
                    calloc->addRetAttr(Attribute::NoAlias);
                }
