...
void PreProcPassManagerBuilder::addFunctionSimplificationPasses(
    ...
    // TODO: this could simplify the loop but might also have bad effect on debug info?
...
void PreProcPassManagerBuilder::populateFunctionPassManager(legacy::FunctionPassManager & FPM) {
...
void PreProcPassManagerBuilder::populateModulePassManager(legacy::PassManagerBase & MPM) {
    ...
    if (runInstCombine) {
        ...
        // it might destroy the bitcast that used for inferring type for heap allocation
        addInstructionCombiningPass(MPM);
    }
    ...
    // we do it after function simplification so that it is easier to understand the wrapper function
