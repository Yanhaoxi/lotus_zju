// from https://github.com/wangteng13/ConfTainter

#include "Dataflow/ConfTainter/ConfTainter.h"
...
vector<pair<CallBase *, Function *> *> getCallerAndCallInst(Function *f)
{
    ...
    for (auto IT = bf_begin(CG), EI = bf_end(CG); IT != EI; IT++)
    {
        ...
        for (CallGraphNode::iterator calledNode_IT = IT->begin(); calledNode_IT != IT->end(); calledNode_IT++)
        {
            ...
            // Extract the actual value from Optional<WeakTrackingVH>
            if (calledNode_IT->first.hasValue())
            {
                if (Value *V = calledNode_IT->first.getValue())
                {
                    if (CallBase *cb = llvm::dyn_cast<CallBase>(V))
                    {
                    ...
                    else
                    {
                        ...
                        llvm::outs() << "STRAGE. NOT a CallBase for the calledNode_IT->first" << getClassType(V) << "\n";
                    }
                }
            }
        }
                  ...
                  unsigned level)
{
    ...
    if (DIBasicType *basic_type = dyn_cast<DIBasicType>(di_type))
    {
        ...
        {
            MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs() << "basic type gv name: " << gv->getName() << " -> " << getOriginalName(gv->getName().str()) << "\n");
    ...
    for (auto i = UserVec.rbegin(), e = UserVec.rend(); i != e; i++)
    {
        ...
        if (Instruction *cur_inst = dyn_cast<Instruction>(cur_user))
        {
            ...
            if (CallBase *call = dyn_cast<CallBase>(cur_inst))
            {
                 ...
                 */
                string func_name = getOriginalName(func->getName().str());
    ...
    for(auto i = UserVec.rbegin(), e = UserVec.rend(); i != e; i++)
    {
        ...
        if (Instruction *cur_inst = dyn_cast<Instruction>(cur_user))
        {
            ...
            if (prev_inst_info == nullptr) // means the first User of current GV.
            {
                ...
                gv_info->currentGVStartingFuncName = getOriginalName(inst_info->InstPtr->getFunction()->getName().str());
