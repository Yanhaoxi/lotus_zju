//===- SoftFloat.cpp - Replace FP operations with softfloat calls
//------------===//
//
...
//===----------------------------------------------------------------------===//
/// \file
/// \brief Soft-float transformation pass implementation
///
/// This pass replaces floating-point instructions with calls to the libgcc
/// softfloat library functions. This is essential for targets that lack
/// native floating-point hardware support.
///
/// The pass handles:
...
// The name will consist of `Prefix`, the `libgcc` identifiers of `ArgTy` and
// `RetTy`, and `Suffix`.
FunctionCallee getConvert(Module *M, StringRef Prefix, StringRef Suffix,
                          Type *ArgTy, Type *RetTy) {
...
void replaceConvert(Module *M, Instruction &I, StringRef Prefix,
                    StringRef Suffix, bool Signed) {
...
Value *getFCmpReplacement(Module *M, FCmpInst &I) {
  ...
  if (AllowUnord) {
    return BinaryOperator::Create(Instruction::Or, UnordFlag, CmpFlag,
                                  I.getName(), &I);
  } else {
    Value *OrdFlag = BinaryOperator::CreateNot(UnordFlag, I.getName(), &I);
    return BinaryOperator::Create(Instruction::And, OrdFlag, CmpFlag,
                                  I.getName(), &I);
  }
}
