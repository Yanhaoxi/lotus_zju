/**
 * VecSet data structure for FPSolve
 */

#ifndef FPSOLVE_VEC_SET_H
#define FPSOLVE_VEC_SET_H
...
class VecIter : public boost::iterator_facade<VecIter<Const, A>,
                                              typename detail::AddConst<Const, A>::type,
                                              boost::forward_traversal_tag> {
  typedef typename detail::AddConst< Const, std::vector< detail::ElemInfo<A> > >::type Vector_;
  typedef typename detail::AddConst< Const, A >::type A_;
  
  public:
    VecIter() : vector_(nullptr), index_(detail::nil_index) {}

    explicit VecIter(Vector_ *v, detail::Index i)
        : vector_(v), index_(i) {}
...
class VecSet {
  public:
    ...
    VecSet(std::vector<A> &&v) {
      ...
      detail::Index next = 0;
      for (detail::Index i = 0; i < v.size(); ++i) {
        /* Skip duplicates. */
    ...
    bool Contains(const A &elem) const {
      struct Less {
        bool operator()(const detail::ElemInfo<A> &lhs, const A &rhs) {
    ...
    iterator MarkErased(iterator iter) {
      assert(marked_erased == detail::nil_index);
      detail::ElemInfo<A> &ei = vector_[iter.index_];
      ei.erased = true;
    ...
    void CommitErase() {
      marked_erased = detail::nil_index;
      assert(Ok());
    }

    void AbortErase() {
      assert(marked_erased != detail::nil_index);
      detail::ElemInfo<A> &ei = vector_[marked_erased];
      ei.erased = false;
    ...
    void emplace_back(A &&a) {
      assert(marked_erased == detail::nil_index);
      detail::Index index = vector_.size();
      detail::Index prev_index = (index == 0 ? detail::nil_index : index - 1);
      vector_.emplace_back(detail::ElemInfo<A>{std::move(a), prev_index, index + 1});
      assert(Ok());
    }
  ...
  private:
    ...
    bool Ok() const {
      ...
      detail::Index prev = begin_;
      for (detail::Index current = begin_;
           current < vector_.size();
           prev = current, current = vector_[current].next) {
        if (current >= vector_.size()) {
          DMSG("VecSet: Ok: corrupted index");
          Debug();
          return false;
        }
        if (current == begin_ && vector_[current].prev != detail::nil_index) {
    ...
    std::vector< detail::ElemInfo<A> > vector_;
    detail::Index begin_ = 0;
    detail::Index marked_erased = detail::nil_index;
    detail::Index num_erased_ = 0;
};
...
template<typename A>
struct hash< fpsolve::VecSet<A> > {
  inline std::size_t operator()(const fpsolve::VecSet<A> &set) const {
    std::size_t h = 0;
    for (auto &x : set) {
