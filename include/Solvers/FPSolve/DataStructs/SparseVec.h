/**
 * Sparse vector for FPSolve
 */

#ifndef FPSOLVE_SPARSE_VEC_H
#define FPSOLVE_SPARSE_VEC_H
...
template<typename Var, typename Value, DIVIDER_TEMPLATE_TYPE Divider>
struct hash< fpsolve::SparseVec<Var, Value, Divider> > {
  inline std::size_t operator()(const fpsolve::SparseVec<Var, Value, Divider> &vec) const {
    return vec.Hash();
  }
};
