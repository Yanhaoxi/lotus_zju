...
public:
	MatrixUblas(const MatrixUblas& matrix)
	{
		this->m = matrix.m;
	}

	MatrixUblas(int c, int r, std::initializer_list<SR> elements)
	{
	...
	MatrixUblas& operator=(const MatrixUblas& matrix)
	{
		this->m = matrix.m;
		return (*this);
	};
	...
	MatrixUblas(int c, int r, const SR& elem){ // initialize c x r matrix with elem
		this->m = ub::matrix<SR>(r,c);
		for(int i_r = 0; i_r < r; ++i_r)
		{
			for(int i_c = 0; i_c < c; ++i_c)
	...
	MatrixUblas(unsigned int c, unsigned int r, const std::vector<SR>& elements)
	{
	...
	MatrixUblas operator += (const MatrixUblas& mat)
	{
		this->m = this->m + mat.m;
		return *this;
	};
	MatrixUblas operator + (const MatrixUblas& mat) const
	{
		MatrixUblas result = *this;
		result += mat;
		return result;
	}
	MatrixUblas operator *= (const MatrixUblas& mat)
	...
	bool operator== (const MatrixUblas& mat)
	{
	...
	static std::shared_ptr<MatrixUblas<SR>> elem_null;
	static std::shared_ptr<MatrixUblas<SR>> elem_one;

	static MatrixUblas<SR> const null(int size)
	{
		if(!MatrixUblas::elem_null)
			MatrixUblas::elem_null = std::shared_ptr<MatrixUblas<SR>>(new MatrixUblas(size, size, SR::null()));
		return *MatrixUblas::elem_null;
	}

	static MatrixUblas<SR> const one(int size)
	{
		if(!MatrixUblas::elem_one)
		{
			std::vector<SR> ret;
			...
			MatrixUblas::elem_one = std::shared_ptr<MatrixUblas<SR>>(new MatrixUblas(size, size, ret));
		}
		return *MatrixUblas::elem_one;
	}
};

template <typename SR> std::shared_ptr<MatrixUblas<SR>> MatrixUblas<SR>::elem_null;
template <typename SR> std::shared_ptr<MatrixUblas<SR>> MatrixUblas<SR>::elem_one;

template <typename SR>
std::ostream& operator<<(std::ostream& os, const MatrixUblas<SR>& matrix)
{
