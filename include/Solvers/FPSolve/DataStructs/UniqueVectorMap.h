...
class UniqueVMapBuilder {
  ...
  public:
    ...
    UniqueVMapPtr<K, V> NewDiff(const UniqueVMap<K, V> &lhs,
      ...
      /* We must go through all the elements of rhs (but not necessarily of
...
template<typename K, typename V>
struct hash< fpsolve::UniqueVMap<K, V> > {
  inline std::size_t operator()(const fpsolve::UniqueVMap<K, V> &vec_map) const {
    return vec_map.Hash();
  }
};

template<typename K, typename V>
struct hash< fpsolve::UniqueVMapPtr<K, V> > {
  inline std::size_t operator()(const fpsolve::UniqueVMapPtr<K, V> &ptr) const {
    std::hash<const fpsolve::UniqueVMap<K, V>*> h;
    return h(ptr.get());
  }
};
...
#endif // FPSOLVE_UNIQUE_VECTOR_MAP_H
