#ifndef FPSOLVE_LINEAR_SET_H
#define FPSOLVE_LINEAR_SET_H
...
struct hash< fpsolve::LinearSet<S, V> > {
  inline std::size_t operator()(const fpsolve::LinearSet<S, V> &set) const {
    return set.Hash();
  }
