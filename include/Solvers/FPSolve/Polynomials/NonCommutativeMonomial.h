/**
 * Non-commutative monomial for FPSolve
 */
...
public:
  ...
  NonCommutativePolynomialBase<SR> derivative(const SubstitutionMap &substitution) const {
    ...
    for(unsigned int position = 0; position < this->variables_.size(); position++) {
      ...
      tmp.variables_.at(position) = this->variables_.at(position); // restore this one
      result += tmp;
    }
    return result;
  }
  ...
  SR calculate_delta_helper(
    ...
    for(auto const &p : this->idx_) {
      if(p.first == Variable) {
        if(permutation.at(p.second) == true) {
  ...
  SR calculate_delta(
    ...
    {
      std::vector<bool> permutation(n, false);
      std::vector<bool> permutation2(this->variables_.size()-n, true);
...
#endif // FPSOLVE_NON_COMMUTATIVE_MONOMIAL_H
