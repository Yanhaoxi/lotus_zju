...
private:
    ...
    llvm::DenseMap<KeyType, const InDirectCallSite<ctx> *> ctxFunPtrMap;  // indirect call sites
    ...
    template <typename BeforeNewNode, typename OnNewDirectNode, typename OnNewInDirect>
    inline std::pair<CallNodeTy *, bool> getOrCreateCallNode(const ctx *curCtx, const ctx *preCtx,
                                                             const llvm::Function *F,
                                                             const llvm::Instruction *I,
                                                             BeforeNewNode beforeNewNode,
                                                             OnNewDirectNode onNewNode,
                                                             OnNewInDirect onNewInDirect) {
        // check the signature
        // TODO: add it back!
        // std::is_invocable_r is only available on C++17
        /*
        static_assert(std::is_invocable_r<bool, OnNewDirectNode, CallNodeTy *>::value, "");
        static_assert(
            std::is_invocable_r<InterceptResult, BeforeNewNode, const llvm::Function **, const llvm::Instruction
        *>::value);
        */
        bool isEntryNode = false;
        if (I == nullptr) {
            assert(F->getName().equals(entryName) && "empty call site for non-entry function??");
            isEntryNode = true;
        }
        ...
        if (isEntryNode) {
            ...
            auto result = ctxFunMap.insert(std::make_pair(std::make_pair(curCtx, F),  // ctx + llvm::Function
                                                          callNode->getTargetFun())).second; // CtxFunction
            assert(result);
            ...
            // now allow user to intercept the entry function, we did it after all pta node are added
            // so that they are accessible by the user
        ...
        if (llvm::isa<llvm::Function>(interceptResult.redirectTo)) {
            ...
            // new node are inserted
            CallNodeTy *callNode = callGraph->createCallNode(curCtx, F, I);
            auto result = ctxFunMap.insert(std::make_pair(std::make_pair(curCtx, F), callNode->getTargetFun())).second;
            assert(result);

            auto fun = const_cast<CtxFunction<ctx> *>(callNode->getTargetFun());
            bool needToExpand = false;
            if (interceptResult.option == InterceptResult::Option::EXPAND_BODY) {
                needToExpand = onNewNode(callNode);  // the call node might be an external function
            }
            ...
            CallNodeTy *callNode = getOrCreateInDirectCallNode(preCtx, I, onNewInDirect, indirect).first;
            return std::make_pair(callNode, false);
        }
    }

    template <typename OnNewInDirectNode>
    inline std::pair<CallNodeTy *, bool> getOrCreateInDirectCallNode(const ctx *C, const llvm::Instruction *I,
        ...
        auto result = ctxFunPtrMap.insert(std::make_pair(std::make_pair(C, I), callNode->getTargetFunPtr())).second;
        assert(result);
    ...
    inline void createEdge(CallNodeTy *src, CallNodeTy *dst, const llvm::Instruction *inst, OnNewEdge callBack) {
        ...
        // check the signature
        // TODO: add it back
        // static_assert(std::is_invocable<OnNewEdge, CallNodeTy *, CallNodeTy *, const llvm::Instruction *>::value,
        // "");
    ...
    inline void expandFromNode(CallNodeTy *root, BeforeNewNode beforeNewNode, OnNewDirectNode onNewDirect,
        ...
        while (!BFSQueue.empty()) {
            ...
            BFSQueue.pop();
            visitCallNode(curNode, BFSQueue, beforeNewNode, onNewDirect, onNewInDirect, onNewEdge);
        }
    }

    // Assumption: the call node has never been visited before
    template <typename BeforeNewNode, typename OnNewDirectNode, typename OnNewInDirectNode, typename OnNewEdge>
    void visitCallNode(CallNodeTy *node, std::queue<CallNodeTy *> &BFSQueue, BeforeNewNode beforeNewNode,
                       OnNewDirectNode onNewDirect, OnNewInDirectNode onNewInDirect, OnNewEdge onNewEdge) {
    ...
    template <typename OnNewInDirectNode>
    inline CallNodeTy *visitInDirectCallSite(const llvm::Instruction *callSite, const ctx *context,
        ...
        auto r = this->getOrCreateInDirectCallNode(context, callSite, onNewInDirect);
        return r.first;
    }

    template <typename BeforeNewNode, typename OnNewDirectNode, typename OnNewInDirect>
    inline CallNodeTy *visitDirectCallSite(const ctx *curCtx, const ctx *preCtx,
                                           const llvm::Function *fun, const llvm::Instruction *I,
                                           std::queue<CallNodeTy *> &BFSQueue, BeforeNewNode beforeNewNode,
                                           OnNewDirectNode onNewDirect, OnNewInDirect onNewInDirect) {
        assert(fun != nullptr);
        auto r = this->getOrCreateCallNode(curCtx, preCtx, fun, I,
                                           beforeNewNode, onNewDirect, onNewInDirect);
...
public:
    ...
    // OnNewNode: call back
    template <typename BeforeNewNode, typename OnNewDirectNode, typename OnNewInDirectNode, typename OnNewEdge>
    void buildInitCallGraph(BeforeNewNode beforeNewNode, OnNewDirectNode onNewDirect, OnNewInDirectNode onNewIndirect,
                            OnNewEdge onNewEdge) {
        // TODO: add it back
        /*
        static_assert(std::is_invocable_r<bool, OnNewDirectNode, CallNodeTy *>::value, "");
        static_assert(std::is_invocable_r<void, OnNewInDirectNode, CallNodeTy *>::value, "");
        static_assert(
            std::is_invocable_r<void, OnNewEdge, CallNodeTy *, CallNodeTy *, const llvm::Instruction *>::value, "");
        */
        ...
        if (entryFun == nullptr || entryFun->isDeclaration()) {
            llvm::errs() << "Fatal Error: '" << entryName << "' function cannot be found!\n";
            exit(1);
        }
        // here do the initial call callgraph construction
        // main function has not call site
        auto entryNode = getOrCreateCallNode(initialCtx, initialCtx, entryFun, nullptr,
                                             beforeNewNode, onNewDirect, onNewIndirect).first;
        // CallNode *entryNode = result.first;
        expandFromNode(entryNode, beforeNewNode, onNewDirect, onNewIndirect, onNewEdge);
    }

    template <typename BeforeNewNode, typename OnNewDirectNode, typename OnNewInDirectNode, typename OnNewEdge>
    void resolveCallTo(CallGraphNode<ctx> *resolvedNode, const llvm::Function *target,
        ...
        */
        ...
        for (; it != et; it++) {
            ...
            const ctx *calleeCtx = CT::contextEvolve(callerNode->getContext(), callInst);
            ...
            if (result.first == nullptr) {
                ...
                if (result.second) {
                    ...
                    expandFromNode(result.first, beforeNewNode, onNewDirect, onNewIndirect, onNewEdge);
                }
            }
        }
    }
