...
public:
    ...
    inline void interceptHeapAllocSite(const CtxFunction<ctx> *caller, const CtxFunction<ctx> *callee,
        ...
        llvm::Type *heapObjType = inferHeapType(caller, callee, allocSite);
...
private:
    // Helper to infer heap type - uses SFINAE instead of constexpr if
    template<typename MMT_ = MMT>
    typename std::enable_if<MMT_::NEED_TYPE_INFO, llvm::Type*>::type
    inferHeapType(const CtxFunction<ctx> *caller, const CtxFunction<ctx> *callee,
                  const llvm::Instruction *allocSite) {
        llvm::Type *heapObjType = heapModel.inferHeapAllocType(callee->getFunction(), allocSite);
    ...
    template<typename MMT_ = MMT>
    typename std::enable_if<!MMT_::NEED_TYPE_INFO, llvm::Type*>::type
    inferHeapType(const CtxFunction<ctx> *, const CtxFunction<ctx> *,
