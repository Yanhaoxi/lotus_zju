...
#include "Alias/AserPTA/PointerAnalysis/Graph/GraphBase/GraphBase.h"
...
struct GraphTraits<aser::CallGraph<ctx>>
    : public GraphTraits<aser::GraphBase<aser::CallGraphNode<ctx>, aser::CallEdge>> {};

template <typename ctx>
struct GraphTraits<const aser::CallGraph<ctx>>
    : public GraphTraits<const aser::GraphBase<aser::CallGraphNode<ctx>, aser::CallEdge>> {};

template <typename ctx>
struct DOTGraphTraits<const aser::CallGraph<ctx>> : public DefaultDOTGraphTraits {
    explicit DOTGraphTraits(bool simple = false) : DefaultDOTGraphTraits(simple) {}

    static std::string getGraphName(const aser::CallGraph<ctx> &) { return "CallGraph"; }

    /// Return function name;
    static std::string getNodeLabel(const aser::CallGraphNode<ctx> *node, const aser::CallGraph<ctx> &graph) {
        std::string str;
    ...
    static bool isNodeHidden(const aser::CallGraphNode<ctx> *node, const aser::CallGraph<ctx> &graph) {
        return false;  // Show all nodes by default
    }
};
