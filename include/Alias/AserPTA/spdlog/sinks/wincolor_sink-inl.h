...
#include <Alias/AserPTA/spdlog/sinks/wincolor_sink.h>
#endif

#include <Alias/AserPTA/spdlog/common.h>
#include <Alias/AserPTA/spdlog/details/pattern_formatter.h>
...
SPDLOG_INLINE wincolor_sink<ConsoleMutex>::wincolor_sink(HANDLE out_handle, color_mode mode)
    ...
    // ::GetConsoleMode() should return 0 if it is redirected or not valid
    // console handle.
    DWORD console_mode;
...
void SPDLOG_INLINE wincolor_sink<ConsoleMutex>::write_to_file_(const memory_buf_t &formatted) {
    ...
    do {
        ...
        bool ok = ::WriteFile(out_handle_, formatted.data() + total_written, size - total_written, &bytes_written,
                              nullptr) != 0;
        if (!ok || bytes_written == 0) {
            SPDLOG_THROW(
                spdlog_ex("wincolor_sink: write_to_file_ failed. GetLastError(): " + std::to_string(::GetLastError())));
        }
