...
#include <Alias/AserPTA/spdlog/spdlog.h>
#endif
...
SPDLOG_INLINE void flush_every(std::chrono::seconds interval) { details::registry::instance().flush_every(interval); }
...
SPDLOG_INLINE void enable_log_on_file(const char *file) { details::registry::instance().enabled_files.insert(file); }
