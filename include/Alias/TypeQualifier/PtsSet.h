...
public:
  ...
  void reset(unsigned idx) { bitvec.reset(idx); }
  ...
  bool unionWith(AndersPtsSet &other) { return bitvec |= other.bitvec; }
  ...
  bool isEmpty() const {
    return bitvec
        .empty(); // Always prefer using this function to perform empty test
  }
