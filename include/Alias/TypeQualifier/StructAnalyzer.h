...
private:
  ...
  void addField(unsigned newFieldSize, bool isArray, bool isUnion,
                bool isPointer) {
  ...
  void finalize() {
    ...
    if (stType->isSized())
      allocSize =
          dataLayout->getTypeAllocSize(const_cast<llvm::StructType *>(stType));
    else
      allocSize = 0;
    finalized = true;
  }
...
public:
  ...
  const StructInfo *getStructInfo(const llvm::StructType *st,
                                  llvm::Module *M) const;
