...
#include "Alias/Pestrie/obstack.h"
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
/* Number of bits in each element in the linked list.  */
#define BLOCK_SIZE (BITMAP_WORD_BITS*2)     // we allow a single WORD in each element

#ifndef BITMAP_ELEMENT_WORDS
#define BITMAP_ELEMENT_WORDS ((BLOCK_SIZE + BITMAP_WORD_BITS - 1) / BITMAP_WORD_BITS)
#endif
...
/*
 * Richard Xiao's Extensions.
 */
#define COMPRESSED_FORMAT 0
#define UNCOMPRESSED_FORMAT 1

// Transform the bitmap into tree form
extern void bitmap_make_tree( bitmap );
// Query if a bit is set in the tree form bitmap
extern int bitmap_tree_query( bitmap, int );
// Count how many bits are in the tree form bitmap
extern unsigned long bitmap_tree_count_bits( bitmap );

// Test if two bitmaps have common bits
extern int bitmap_same_bit_p( bitmap, bitmap );
// Print the bitmap to stdout, call it in GDB can see bitmap content
extern void print_bitmap( bitmap );
// Calculate how much memory is used by this bitmap
extern double bitmap_calculate_memory( bitmap*, int );
// Output the bitmap to external file
extern void bitmap_write_out( bitmap, FILE*, int );
// Read a bitmap from external file
extern bitmap bitmap_read_row( FILE*, int, bool );
// Calculate the labels used by this bitmap in external memory
extern int bitmap_calculate_labels( bitmap*, int );


// First contains second
extern int bitmap_contain_p( bitmap, bitmap );
