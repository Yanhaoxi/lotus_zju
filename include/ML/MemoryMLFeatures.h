...
class MLFeaturesCallSite {
  ...
  // Number of safe allocation sites (potentially) accessed by a memory
  // operation
...
public:
  ...
  MLFeaturesCallSite(llvm::CallBase &CB);

  void write(llvm::raw_ostream &o) const;

  /* Number of nodes in the callee's summary graph */
  unsigned getCalleeNumNodes() const { return callee_nodes; }
  /* Number of nodes in the callee's after top-down unification */
  unsigned getTdCalleeNumNodes() const { return td_callee_nodes; }
  /* Number of read/modified nodes in the callee's summary graph */
  unsigned getCalleeNumAccessed() const { return callee_accessed; }
  /* Number of read/modified nodes in the callee's after top-down unification */
  unsigned getTdCalleeNumAccessed() const { return td_callee_accessed; }
  /* Number of collapsed nodes in the callee's summary graph */
  unsigned getCalleeNumCollapsed() const { return callee_collapsed; }
  /* Number of collapsed nodes in the callee's after top-down unification */
  unsigned getTdCalleeNumCollapsed() const { return td_callee_collapsed; }
  /* Number of sequence nodes in the callee's summary graph */
  unsigned getCalleeNumSequence() const { return callee_sequence; }
  /* Number of sequence nodes in the callee's after top-down unification */
  unsigned getTdCalleeNumSequence() const { return td_callee_sequence; }
  /* Number of stack-allocated nodes in the callee's summary graph */
  unsigned getCalleeNumAlloca() const { return callee_alloca; }
  /* Number of stack-allocated nodes in the callee's after top-down unification
   */
  unsigned getTdCalleeNumAlloca() const { return td_callee_alloca; }
  /* Number of heap-allocated nodes in the callee's summary graph */
  unsigned getCalleeNumHeap() const { return callee_heap; }
  /* Number of heap-allocated nodes in the callee's after top-down unification
   */
  unsigned getTdCalleeNumHeap() const { return td_callee_heap; }
  /* Number of external-allocated nodes in the callee's summary graph */
  unsigned getCalleeNumExternal() const { return callee_external; }
  /* Number of external-allocated nodes in the callee's graph after td
   * unification */
  unsigned getTdCalleeNumExternal() const { return td_callee_external; }
  /* Number of LLVM pointers pointing to some node in the callee's summary graph
   */
  unsigned getCalleeNumPointers() const { return callee_pointers; }
  /* Number of LLVM pointers pointing to some node in the callee's
     graph after td unification*/
  unsigned getTdCalleeNumPointers() const { return td_callee_pointers; }
  ...
  unsigned getCalleeNumAllocSites() const { return callee_alloc_sites; }
  /* Number of allocation sites in the callee's graph after td unification */
  unsigned getTdCalleeNumAllocSites() const { return td_callee_alloc_sites; }
};
...
public:
  ...
  MLFeaturesCallSite extractMLFeatures(llvm::CallBase &CB);
