/**
 * @file SymbolicAbstractionPass.h
 * @brief LLVM function pass that drives the SymbolicAbstraction analyzer and rewrites IR.
 */
#pragma once

#include "Analysis/SymbolicAbstraction/Utils/Utils.h"
#include "Analysis/SymbolicAbstraction/Domains/SimpleConstProp.h"
...
{
/**
 * Runs the SymbolicAbstraction abstract interpreter on a single function and applies
 * local source-level optimizations such as constant propagation and redundant
 * computation elimination to the LLVM IR.
 */
class SymbolicAbstractionPass : public llvm::FunctionPass
{
  public:
    struct Config {
  ...
  private:
     ...
     */
    DomainConstructor getAugmentedDomain(symbolic_abstraction::FunctionContext& smtsem);

  public:
    static char ID;

    SymbolicAbstractionPass();
    ~SymbolicAbstractionPass() {}
