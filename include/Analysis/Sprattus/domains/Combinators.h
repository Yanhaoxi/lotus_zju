...
template <typename T> class Wrapper : public AbstractValue
{
  ...
  public:
    ...
    virtual void prettyPrint(PrettyPrinter& out) const
    {
        Value_->prettyPrint(out);
    }

    virtual bool joinWith(const AbstractValue& other)
    {
    ...
    virtual bool meetWith(const AbstractValue& other)
    {
        auto* spec = static_cast<const Wrapper<T>*>(&other);
        assert(spec);
    ...
    virtual bool updateWith(const ConcreteState& cstate)
    {
        return Value_->updateWith(cstate);
    }

    virtual z3::expr toFormula(const ValueMapping& vmap, z3::context& ctx) const
    {
        return Value_->toFormula(vmap, ctx);
    }

    virtual void havoc() { Value_->havoc(); }
    virtual void resetToBottom() { Value_->resetToBottom(); }
    virtual bool isTop() const { return Value_->isTop(); }
    virtual bool isBottom() const { return Value_->isBottom(); }
    ...
    virtual bool isJoinableWith(const AbstractValue& av_other) const
    {
    ...
    virtual bool operator<=(const AbstractValue& av_other) const
    {
        auto* other = static_cast<const Wrapper<T>*>(&av_other);
...
class If : public AbstractValue
{
  ...
  public:
    ...
    virtual bool joinWith(const AbstractValue& av_other)
    {
...
#ifdef ENABLE_DYNAMIC
CEREAL_REGISTER_TYPE(sprattus::domains::If);
#endif
