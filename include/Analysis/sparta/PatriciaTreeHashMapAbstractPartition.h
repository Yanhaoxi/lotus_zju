/*
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */

#pragma once

#include <Analysis/sparta/AbstractPartition.h>
#include <Analysis/sparta/PatriciaTreeHashMap.h>

namespace sparta {

/*
 * An abstract partition based on `PatriciaTreeHashMap`.
 *
 * See `AbstractPartition` for more information.
 */
template <typename Label, typename Domain>
using PatriciaTreeHashMapAbstractPartition = AbstractPartition<
    PatriciaTreeHashMap<Label, Domain, BottomValueInterface<Domain>>>;

} // namespace sparta
