Applications
============

This section covers the high-level applications and tools built on Lotus, organized to match the structure of ``lib/Apps/``.

Components
----------

* **Checker** – Bug detection framework (``lib/Apps/Checker/``)
* **CLAM** – Abstract interpretation-based analyzer (``lib/Apps/clam/``)
* **Fuzzing** – Directed greybox fuzzing support (``lib/Apps/Fuzzing/``)
* **MCP** – Model Context Protocol server for call graph analysis (``lib/Apps/MCP/``)
* **SeaHorn** – SMT-based verification framework (``lib/Apps/seahorn/``)

.. toctree::
   :maxdepth: 2

   checker/index
   clam
   fuzzing_support
   mcp
   seahorn
