# Sprattus Configuration Guide

Everything you need to run Sprattus lives in this directory. Pick a config, point `spranalyze` at it, and go.

## TL;DR
- `spranalyze --list-configs` shows the menu.
- `spranalyze --config config/sprattus/FILE.conf input.bc` is the common path.
- `--abstract-domain NAME` and `--function main` override config values.
- Set `SPRATTUS_CONFIG=config/sprattus/FILE.conf` to make a default.
...
spranalyze --config config/sprattus/03_const_edge.conf program.bc
spranalyze --abstract-domain SimpleConstProp --function main program.bc
spranalyze --verbose --config config/sprattus/17_const_function_verbose.conf debug.bc
```
...
- **Audit memory:** `spranalyze --config config/sprattus/memsafety_sv16_body.conf unsafe.bc`
...
- Custom parser in `lib/Analysis/Sprattus/Config.cpp` loads plain `key=value` files and respects `SPRATTUS_CONFIG`.
...
- Load a few configs: `spranalyze --config config/sprattus/01_const_function.conf test.bc`
...
- `config/sprattus/*.conf` – ready-to-use profiles (see headers in each file).
- `tools/sprattus/spranalyze.cpp` – CLI entry point with option docs.
